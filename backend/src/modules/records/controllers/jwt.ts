...
async function validateToken(token: string, secret: string): Promise<any> {
...
async function validateRefreshToken(token: string): Promise<JwtTokenPayload> {
    return await validateToken(token, secrets.refresh);
}

async function validateAccessToken(token: string): Promise<JwtTokenPayload> {
